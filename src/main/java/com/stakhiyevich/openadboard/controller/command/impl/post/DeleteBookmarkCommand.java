...
import static com.stakhiyevich.openadboard.util.MessageKey.MESSAGE_DELETE_BOOKMARK_FAIL;
import static com.stakhiyevich.openadboard.util.MessageKey.MESSAGE_DELETE_BOOKMARK_SUCCESS;
...
public class DeleteBookmarkCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) {
        ...
        if (user != null) {
            result = bookmarkService.deleteBookmark(user.getId(), itemId);
        }

        if (result) {
            session.setAttribute(ADD_DELETE_BOOKMARK_FEEDBACK, MESSAGE_DELETE_BOOKMARK_SUCCESS);
        } else {
            session.setAttribute(ADD_DELETE_BOOKMARK_FEEDBACK, MESSAGE_DELETE_BOOKMARK_FAIL);
        }
